...
class ReferralLink
{
	...
	{
		\wp_register_script('wr-referral-link', LWS_WOOREWARDS_PRO_JS . '/shortcodes/referral-link.js', array('jquery'), LWS_WOOREWARDS_PRO_VERSION);
		\wp_register_style('wr-referral-link', LWS_WOOREWARDS_PRO_CSS . '/shortcodes/referral-link.min.css', array(), LWS_WOOREWARDS_PRO_VERSION);
	}
	...
	public function admin($fields)
	{
		$fields['referrallink'] = array(
			...
			'extra' => array(
				...
				'description' =>  __("Use this shortcode to show a referral link button.", 'woorewards-pro') . "<br/>" .
					__("Customers can use then share this link with their friends.", 'woorewards-pro'),
				'options' => array(
					'mode' => array(
						'option' => 'mode',
						'desc' => array(
							__("Select the type of referral link your customers will get :", 'woorewards-pro'),
							array(
								'tag' => 'ul',
								array('link', __("(Default Value) Customers will get an url link", 'woorewards-pro')),
								array('qrcode', __("Customers will get a QR Code link", 'woorewards-pro')),
							),
						),
					...
					'showlink' => array(
						'option' => 'showlink',
						'desc' => __("(Optional) If set, customers will see the referral link or QR-Code above the copy button", 'woorewards-pro'),
						'example' => '[wr_referral_link showlink="true"]',
					),
					'showbutton' => array(
						'option' => 'showbutton',
						'desc' => __("(Optional, default is 'true') Show or hide the copy button", 'woorewards-pro'),
						'example' => '[wr_referral_link showbutton="true"]',
					),
					'url' => array(
						'option' => 'url',
						'desc' => __("(Optional) By default, the shortcode shares the url of the page it’s displayed on. You can override that setting by setting an url in this option.", 'woorewards-pro'),
						'example' => '[wr_referral_link url="https://mywebsite.com/nameofapage"]',
					),
					'button' => array(
						'option' => 'button',
						'desc' => __("(Optional) Set the text customers will see on the copy button", 'woorewards-pro'),
						'example' => '[wr_referral_link button="Copy my referral link"]',
					),
					'copied' => array(
						'option' => 'copied',
						'desc' => __("(Optional) Set the text customers will see when the code is copied to the clipboard", 'woorewards-pro'),
	...
	{
		$showlnk = (\LWS\Adminpanel\Tools\Conveniences::argIsTrue($atts['showlink']) ? '' : ' hidden');
		...
		if ($atts['mode'] == 'link') {
			...
			if (!$atts['button']) {
				$atts['button'] = __('Get your referral link', 'woorewards-pro');
			}
		} else {
			// QR Code
			$link = \esc_attr($url);
			...
			if (!$atts['button']) {
				$atts['button'] = __('Get your referral QR Code', 'woorewards-pro');
			}
		}
		if (!$atts['copied']) {
			$atts['copied'] = __('Your code has been copied !', 'woorewards-pro');
		}
