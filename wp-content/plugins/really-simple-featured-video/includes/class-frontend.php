		...
		if ( ! empty( $post_types ) ) {
			if ( in_array( $product->post_type, $post_types, true ) && 0 === $this->counter ) {
				...
				if ( 'self' === $video_source ) {
					...
					if ( $video_url ) {
						$html = '<div class="woocommerce-product-gallery__image rsfv-video__wrapper" data-thumb="' . RSFV_PLUGIN_URL . 'assets/images/video_frame.png"><video class="rsfv-video" playsinline id="rsfv_video_' . $product->get_id() . '" src="' . $video_url . '" style="max-width:100%;display:block;"' . "{$has_controls} {$is_autoplay} {$is_loop} {$is_muted} {$is_pip}" . '></video></div>' . $html;
					}
				} else {
					// Get the meta value of video embed url.
