...
<p>
<?php
	esc_html_e( 'We just need a few details from you to complete your account creation.', 'woocommerce-subscriptions-gifting' );
?>
<br />
	<?php
	printf(
		/* translators: 1$: user's email, 2$-3$: opening and closing link tags, logs the user out. */
		esc_html__( '(not %1$s? %2$sSign out%3$s)', 'woocommerce-subscriptions-gifting' ),
...
foreach ( $form_fields as $key => $field ) {
	if ( 'shipping_company' === $key ) {
		...
		<h3>
		<?php
		esc_html_e( 'Address Detail', 'woocommerce-subscriptions-gifting' );
		?>
		</h3>
		<?php
	}
