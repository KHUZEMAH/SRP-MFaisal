...
class WCS_Gifting {
			...
			if ( empty( $item['wcsg_gift_recipients_email'] ) || $item['wcsg_gift_recipients_email'] != $new_recipient_data ) { // phpcs:ignore WordPress.PHP.StrictComparisons.LooseComparison
				...
				if ( $new_key != $key ) { 
					...
					if ( ! empty( $cart_item ) ) {
						...
						$one =	array_slice( WC()->cart->cart_contents, 0, $item_cart_position, true );
						$two = 	array( $new_key => WC()->cart->cart_contents[ $key ] );
						$three =	array_slice( WC()->cart->cart_contents, $item_cart_position, count( WC()->cart->cart_contents ), true );
	 ...
	 * @since 2.1
	 */
	public static function render_add_recipient_fields( $email = '', $recipient_name = '', $recipient_message = '', $id = '', $print_or_return = 'print' ) {
		
		$output = wc_get_template_html(
			'html-add-recipient.php',
			self::get_add_recipient_template_args( $email, $recipient_name, $recipient_message, $id ),
			'',
	 ...
	 * @since 2.1
	 */
	public static function get_add_recipient_template_args( $email = '', $recipient_name = '', $recipient_message = '', $id = '' ) {
		...
		$recipient_name_field_args = array(
			'placeholder'      => 'John Doe',
			'class'            => array( 'woocommerce_subscriptions_gifting_recipient_name' ),
			'style_attributes' => array(),
		);
		
		// Recipient Message Field.
		$recipient_message_field_args = array(
			'placeholder'      => 'Enjoy your class',
			'class'            => array( 'woocommerce_subscriptions_gifting_recipient_message' ),
			'style_attributes' => array(),
		);
		...
		$args = array(
			...
			'recipient_name_field_args'  => apply_filters( 'wcsg_recipient_name_field_args', $recipient_name_field_args, $recipient_name ),
			'recipient_message_field_args'  => apply_filters( 'wcsg_recipient_message_field_args', $recipient_message_field_args, $recipient_message ),
	...
	public static function get_recipient_email_field_args( $email ) {
		...
		$args = self::get_add_recipient_template_args( $email, $recipient_name );
