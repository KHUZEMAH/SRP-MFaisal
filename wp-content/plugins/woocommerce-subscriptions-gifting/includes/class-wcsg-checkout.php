...
class WCSG_Checkout {
			...
			if ( ! empty( $_POST['_wcsgnonce'] ) && wp_verify_nonce( $_POST['_wcsgnonce'], 'wcsg_add_recipient' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, WordPress.Security.ValidatedSanitizedInput.MissingUnslash
				...
				foreach ( WC()->cart->cart_contents as $key => $item ) {
					if ( isset( $_POST['recipient_email'][ $key ] ) ) {
						WCS_Gifting::update_cart_item_key( $item, $key, $_POST['recipient_email'][ $key ], $field="recipient_email" ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, WordPress.Security.ValidatedSanitizedInput.MissingUnslash
					}
					
					if ( isset( $checkout_data['recipient_email'][ $key ] ) &&  isset( $_POST['recipient_name'][ $key ] ) ) {
						WCS_Gifting::update_cart_item_key( $item, $key, $_POST['recipient_name'][ $key ], $field="recipient_name" ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, WordPress.Security.ValidatedSanitizedInput.MissingUnslash
					}
					
					if ( isset( $checkout_data['recipient_email'][ $key ] ) &&  isset( $_POST['recipient_message'][ $key ] ) ) {
						WCS_Gifting::update_cart_item_key( $item, $key, $_POST['recipient_message'][ $key ], $field="recipient_message" ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, WordPress.Security.ValidatedSanitizedInput.MissingUnslash
					}
				}
			} else {
	...
	public static function store_recipients_in_session( $checkout_data ) {
		...
		//var_dump($checkout_data);

		if ( isset( $checkout_data['recipient_email'] ) ) {
			
			// loop to get all product ids
			...
			foreach( WC()->cart->get_cart() as $nonce_key => $cart_item ) {
				...
				$vals = array_count_values($products_ids_array);
				
				// if same product exists multiple times in cart
				if($vals[$cart_item['product_id']] > 1){
					
					// check duplicate email, i.e we can not allowed same email for two same producs
					if (++$dupe_array[$checkout_data['recipient_email'][$nonce_key]] > 1 ) {
					
						wc_add_notice( __( 'Can not add another recipient with same email.', 'your-textdomain' ), 'error' );	
						$duplicate_email++;							
					}	
				}		
			...
			// only update cart items when above condition not satisfied
			if($duplicate_email == 0){
				// Store recipient emails on the cart items so they can be repopulated after checkout update.
				foreach ( WC()->cart->cart_contents as $key => $item ) {
					if ( isset( $checkout_data['recipient_email'][ $key ] ) && $checkout_data['recipient_email'][ $key ] != ""   ) {
						WCS_Gifting::update_cart_item_key( $item, $key, $checkout_data['recipient_email'][ $key ], $field="recipient_email" );
					}
					
					if ( isset( $checkout_data['recipient_email'][ $key ] ) && $checkout_data['recipient_email'][ $key ] != "" && isset( $checkout_data['recipient_name'][ $key ] ) ) {
						WCS_Gifting::update_cart_item_key( $item, $key, $checkout_data['recipient_name'][ $key ], $field="recipient_name" ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, WordPress.Security.ValidatedSanitizedInput.MissingUnslash
					}
						
					if ( isset( $checkout_data['recipient_email'][ $key ] ) && $checkout_data['recipient_email'][ $key ] != "" &&  isset( $checkout_data['recipient_message'][ $key ] ) ) {
						WCS_Gifting::update_cart_item_key( $item, $key, $checkout_data['recipient_message'][ $key ], $field="recipient_message" ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, WordPress.Security.ValidatedSanitizedInput.MissingUnslash
					}
				}	
			}
