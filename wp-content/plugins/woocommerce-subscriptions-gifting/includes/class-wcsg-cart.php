...
class WCSG_Cart {
			...
			if ( ! empty( $_POST['_wcsgnonce'] ) && wp_verify_nonce( $_POST['_wcsgnonce'], 'wcsg_add_recipient' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, WordPress.Security.ValidatedSanitizedInput.MissingUnslash
				...
				foreach( WC()->cart->get_cart() as $nonce_key => $cart_item ) {
					...
					if($vals[$cart_item['product_id']] > 1){
						...
						if (++$dupe_array[$_POST['recipient_email'][$nonce_key]] > 1 ) {
				...
				foreach ( WC()->cart->cart_contents as $key => $item ) {
					if ( isset( $_POST['recipient_email'][ $key ] ) && $_POST['recipient_email'] != "" ) {
					...
					if ( isset( $_POST['recipient_name'][ $key ] ) && isset( $_POST['recipient_email'][ $key ] ) && $_POST['recipient_email'] != "" ) {
					...
					if ( isset( $_POST['recipient_message'][ $key ] ) && isset( $_POST['recipient_email'][ $key ] ) && $_POST['recipient_email'] != "" ) {
	...
	public static function maybe_display_gifting_information( $cart_item, $cart_item_key, $print_or_return = 'return' ) {
		...
		if ( self::is_giftable_item( $cart_item ) ) {
			...
			$recipient_name  = ( empty( $cart_item['wcsg_gift_recipients_name'] ) ) ? '' : $cart_item['wcsg_gift_recipients_name'];
			
			
			$recipient_message  = ( empty( $cart_item['wcsg_gift_recipients_message'] ) ) ? '' : $cart_item['wcsg_gift_recipients_message'];
			
			
			//var_dump($cart_item['wcsg_gift_recipients_name']);
			
			$output = WCS_Gifting::render_add_recipient_fields( $email, $recipient_name, $recipient_message, $cart_item_key, 'return' );
