...
class WCSG_Product {
	...
	public static function add_recipient_data( $cart_item_data ) {
		...
		// rezaid customization
		if ( isset( $_POST['recipient_name'] ) && ! empty( $_POST['recipient_name'][0] ) ) {
			$cart_item_data['wcsg_gift_recipients_name'] = wp_unslash( $_POST['recipient_name'][0] );
		}

		if ( isset( $_POST['recipient_message'] ) && ! empty( $_POST['recipient_message'][0] ) ) {
			$cart_item_data['wcsg_gift_recipients_message'] = wp_unslash( $_POST['recipient_message'][0] );
		}	
	...
	public static function get_cart_items_from_session( $item, $values ) {
		...
		// rezaid customization
		if ( array_key_exists( 'wcsg_gift_recipients_name', $values ) ) { // previously added at the product page via $cart_item_data.
			$item['wcsg_gift_recipients_name'] = $values['wcsg_gift_recipients_name'];
			unset( $values['wcsg_gift_recipients_name'] );
		}
		return $item;
	}
	...
	public static function add_gifting_option_product() {
		...
		if ( self::is_giftable( $product ) && ! isset( $_GET['switch-subscription'] ) ) {
			...
			if ( ! empty( $_POST['recipient_name'][0] ) && ! empty( $_POST['_wcsgnonce'] ) && wp_verify_nonce( $_POST['_wcsgnonce'], 'wcsg_add_recipient' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, WordPress.Security.ValidatedSanitizedInput.MissingUnslash
				$recipient_name = $_POST['recipient_name'][0]; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, WordPress.Security.ValidatedSanitizedInput.MissingUnslash
			}
			
			if ( ! empty( $_POST['recipient_message'][0] ) && ! empty( $_POST['_wcsgnonce'] ) && wp_verify_nonce( $_POST['_wcsgnonce'], 'wcsg_add_recipient' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, WordPress.Security.ValidatedSanitizedInput.MissingUnslash
				$recipient_message = $_POST['recipient_message'][0]; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, WordPress.Security.ValidatedSanitizedInput.MissingUnslash
			}
			WCS_Gifting::render_add_recipient_fields( $email, $recipient_name, $recipient_message, $id='' );
		}
	}
