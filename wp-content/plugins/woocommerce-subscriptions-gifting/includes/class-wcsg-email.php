...
class WCSG_Email {
						...
						foreach ( WC()->cart->cart_contents as $key => $item ) {
							if ( isset( $item['wcsg_gift_recipients_email'] ) ) {
								...
								if(isset($item['product_id'])){
									...
									$ingredients_text = get_field('ingredients_and_materials_instruction', $product_id);
									$ingredients_url = get_field('ingredients_and_materials_link', $product_id);
								}
							}
						}
						
						do_action( 'wcsg_new_order_recipient_notification', $recipient_user_id, $recipient_subscriptions, $recipient_name, $recipient_message, $ingredients_text, $ingredients_url );
	 ...
	 * @since 2.1.0
	 */
	public static function generate_new_recipient_user_email( $purchaser_user_id, $recipient_user_id, $recipient_name='test-name', $recipient_message="test-message" ) {
		...
		if ( ! is_wp_error( $key ) ) {
			...
			WC()->mailer();
			do_action( 'wcsg_created_customer_notification', $recipient_user_id, $key, $subscription_purchaser_name, $recipient_name, $recipient_message );
		}
	}
		...
		foreach ( WC()->cart->cart_contents as $key => $item ) {
			if ( isset( $item['wcsg_gift_recipients_email'] ) ) {
				...
				if ( $item['wcsg_gift_recipients_email'] === $new_customer_data['user_email'] ) {
					self::generate_new_recipient_user_email( get_current_user_id(), $customer_id, $recipient_name, $recipient_message );
					break;
				}
			}
		}
