...
printf( esc_html__( '%1$s just purchased %2$s for you at %3$s.', 'woocommerce-subscriptions-gifting' ), wp_kses( $subscription_purchaser, wp_kses_allowed_html( 'user_description' ) ), esc_html( _n( 'a subscription', 'subscriptions', count( $subscriptions ), 'woocommerce-subscriptions-gifting' ) ), esc_html( $blogname ) );
?>
<?php
// Translators: placeholder is either the singular or plural form of "subscription".
printf( esc_html__( ' Details of the %s are shown below.', 'woocommerce-subscriptions-gifting' ), esc_html( _n( 'subscription', 'subscriptions', count( $subscriptions ), 'woocommerce-subscriptions-gifting' ) ) );
?>
</p>


<?php if(isset($recipient_name) && $recipient_name != ''){ ?>
	<p> Recipient Name: <?php echo $recipient_name; ?></p>
<?php } ?>

<?php if(isset($recipient_message) && $recipient_message != ''){ ?>
	<p> Recipient Message: <?php echo  $recipient_message; ?></p>
<?php } ?>
...
<p>
	...
	printf(
		...
		esc_html__( 'You may access your account area to view your new %1$s here: %2$sMy Account%3$s.', 'woocommerce-subscriptions-gifting' ),
...
if(isset($ingredients_text) && $ingredients_text != ''){
	
	echo '<p>'.$ingredients_text.'</p>';
	echo '<p><a target="_blank" href="'.$ingredients_url.'"> '.$ingredients_url.' </a></p>';
